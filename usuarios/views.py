...
# ============================================================
# FUNCIÓN AUXILIAR PARA ENVIAR CORREOS CON SENDGRID
# ============================================================
...
def enviar_correo_sendgrid(
    ...
    try:
        ...
        print(
            f"[CORREO] Excepción enviando correo a "
            f"{destinatario}: {e}"
        )
        ...
        if form.is_valid():
            ...
            if es_empresa:
                ...
                if not plan:
                    ...
                    return render(
                        request,
                        "usuarios/registro_unificado.html",
                        {"form": form}
                    )
...
def iniciar_pago_epayco(request, plan_id):
    ...
    context = {
        ...
        "reference":
            f"plan_{plan.id}_{request.user.id}_{int(time.time())}",
...
def confirmacion_pago_epayco(request):
    ...
    if request.method == "POST":
        ...
        print(
            f"[EPAyCO WEBHOOK] Datos recibidos: {data}"
        )
...
def recuperar_contraseña(request):
    ...
    if request.method == "POST":
        ...
        # ====================================================
        # Usamos SITE_URL para que:
        # - En local apunte a 127.0.0.1:8000
        # - En Render apunte al dominio real

        reset_url = (
            settings.SITE_URL.rstrip("/")
            + reverse(
                "usuarios:cambiar_con_token",
                args=[token]
            )
        )
