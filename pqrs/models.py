...
class Pqrs(models.Model):
    ...
    codigo_radicado = models.CharField(
        max_length=50,
        unique=True,
        verbose_name="Código de Radicado"
    )
    ...
    tipo_detectado = models.CharField(
        max_length=50,
        verbose_name="Tipo Detectado por IA"
    )
...
class RespuestaPqrs(models.Model):
    ...
    id_pqrs = models.ForeignKey(
        Pqrs,
        on_delete=models.CASCADE,
        related_name="respuestas",
        verbose_name="PQRS Asociado"
    )

    id_usuario_responde = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
...
class ChatMensaje(models.Model):
    ...
    def __str__(self):
        return f"{self.usuario.username} - {self.rol} - {self.fecha}"